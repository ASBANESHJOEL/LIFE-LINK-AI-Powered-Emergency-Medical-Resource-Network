      ...
      <div className="glass-panel" style={{ width: '100%', maxWidth: '600px', padding: '36px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              <h2 style={{ fontSize: '1.25rem' }}>Hospital Portal</h2>
              <span className="role-badge badge-HOSPITAL">Hospital Authorized</span>
            </div>
          </div>
        ...
        {organization && (
          <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: '12px', padding: '20px', border: '1px solid var(--bg-card-border)' }}>
            <h3 style={{ fontSize: '0.95rem', color: 'var(--text-muted)', marginBottom: '12px' }}>Affiliated Institution</h3>
            <div style={{ display: 'grid', gridTemplateColumns: '130px 1fr', rowGap: '8px', fontSize: '0.9rem' }}>
              <span style={{ color: 'var(--text-subtle)' }}>Hospital:</span>
              <span style={{ color: '#fff', fontWeight: 600 }}>{organization.hospital?.hospital_name || organization.hospitalId}</span>
              <span style={{ color: 'var(--text-subtle)' }}>Reg ID:</span>
              <span>{organization.hospital?.registration_id || 'Verified'}</span>
              <span style={{ color: 'var(--text-subtle)' }}>Staff Position:</span>
              <span style={{ color: '#3b82f6', fontWeight: 600 }}>{organization.membershipRole}</span>
            </div>
          </div>
        )}
