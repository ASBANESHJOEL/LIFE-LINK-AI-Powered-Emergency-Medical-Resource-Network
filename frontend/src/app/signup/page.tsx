...
export default function SignupPage() {
  ...
  const roleName = role === 'BLOOD_BANK' ? 'Blood Bank' : role.charAt(0) + role.slice(1).toLowerCase();
    ...
    <div className="lifelink-page min-h-screen flex items-center justify-center px-4 py-10">
      <div className="auth-surface p-6 sm:p-9">
        ...
        <div className="mt-6 rounded-lg border border-blue-100 bg-blue-50 px-3.5 py-2.5 flex items-center justify-between text-xs">
          <span className="text-slate-600">Selected role: <strong className="text-slate-900">{roleName}</strong></span>
          <Link href="/choose-role" className="font-semibold text-blue-600">Change</Link>
        </div>
        ...
        <form onSubmit={submit} className="mt-6 space-y-5">
          <div>
            <label htmlFor="signup-email" className="block text-[11px] font-semibold text-slate-700">Mobile number / email</label>
            <input id="signup-email" type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="Enter your details" autoComplete="email" autoFocus className="mt-2 h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10" />
          </div>
          <button disabled={loading} className="h-11 w-full rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold flex items-center justify-center gap-2 disabled:opacity-60">
